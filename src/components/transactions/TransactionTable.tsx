    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[900px] table-fixed text-sm">
        <colgroup>
          ...
          <col className="w-44" />   {/* Category */}
          <col className="w-40" />   {/* Notes */}
          <col className="w-[88px]" />{/* One-time */}
          <col className="w-14" />   {/* Hide */}
        </colgroup>
        <thead>
        ...
        <tbody className="divide-y divide-gray-50 dark:divide-gray-800/60">
          {transactions.map((t) => (
            ...
            >
                ...
                <div className="flex items-center justify-center gap-1.5">
                  {anomalies?.has(t.flag_id) && (
                    <span
                      ...
                      className="rounded px-1 py-0.5 text-[10px] font-semibold bg-amber-50 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400"
                    >
                      !</span>
                  )}
              ...
              <td className="px-6 py-3">
                <span className={`inline-block rounded-md px-2 py-0.5 text-xs font-medium capitalize ${SOURCE_COLORS[t.source] ?? ""}`}>
                  {t.source}
                </span>
              </td>
              <td className="px-6 py-3">
                <CategoryDropdown
                  ...
                  onChange={(subcategory) =>
                    setFlag.mutate({ flagId: t.flag_id, update: { month, subcategory } })
                  }
                />
              </td>
              <td className="px-6 py-3">
                <Tooltip label={t.notes} className="block">
                  <input
                    ...
                    onBlur={(e) =>
                      ...
                      setFlag.mutate({ flagId: t.flag_id, update: { month, notes: e.target.value } })
                    }
              ...
              <td className="px-6 py-3 text-center">
                <input
                  ...
                  checked={t.one_time}
                  onChange={(e) =>
                    setFlag.mutate({ flagId: t.flag_id, update: { month, one_time: e.target.checked } })
                  }
